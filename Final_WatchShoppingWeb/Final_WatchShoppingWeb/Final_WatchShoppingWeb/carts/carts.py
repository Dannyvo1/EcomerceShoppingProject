...
def Addcart():
    try:
        ...
        if 'product_id' in request.form and 'quantity' in request.form and 'colors' in request.form and request.method == "POST":
            DicItems = {product_id:{'name': product.name, 'price': float(product.price), 'discount': product.discount, 'color': product.colors, 'quantity': quantity, 'image': product.image_1}}
